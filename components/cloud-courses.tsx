            ...
            return <div key={course.id} className="border-b border-slate-100 last:border-0">
              <div className="grid gap-4 px-5 py-4 hover:bg-slate-50/70 lg:grid-cols-[minmax(260px,2.5fr)_minmax(88px,1fr)_minmax(150px,1.3fr)_minmax(160px,1.3fr)_minmax(260px,1.8fr)] lg:items-center">
                ...
                <div className="flex min-w-0 flex-nowrap items-center gap-2 lg:justify-end">
                  {job?.status === 'succeeded' && job.downloadUrl && <button type="button" onClick={() => void downloadVideo(job)} className="inline-flex h-8 shrink-0 items-center gap-1.5 whitespace-nowrap rounded-lg border border-emerald-200 bg-emerald-50 px-2.5 text-xs font-medium text-emerald-700 transition-colors hover:border-emerald-300 hover:bg-emerald-100"><Download className="size-3.5" />下载</button>}
