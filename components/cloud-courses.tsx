...
function CourseStatus({
  ...
  state: CourseSaveState;
  generationStatus: CloudCourse['generation_status'];
}) {
  ...
  if (generationStatus === 'failed' || generationStatus === 'conflict') {
    return (
...
export default function CloudCourses() {
  ...
  useEffect(() => {
    const closeMenu = (event?: PointerEvent) => {
      if (!event || !actionMenuRef.current?.contains(event.target as Node)) setActionMenuId(null);
    };
  ...
  const hasActiveVideoExport = useMemo(
    () =>
      Object.values(videoExports)
        .flat()
        .some((job) => job.status === 'queued' || job.status === 'rendering'),
    [videoExports],
  );
  const hasActiveCourseGeneration = useMemo(
    () =>
      courses.some(
        (course) => course.generation_status === 'queued' || course.generation_status === 'running',
      ),
    [courses],
  );

  useEffect(() => {
    if (!hasActiveVideoExport && !hasActiveCourseGeneration) return;
    const timer = window.setInterval(() => void fetchCourses(), 10_000);
    return () => window.clearInterval(timer);
  }, [fetchCourses, hasActiveCourseGeneration, hasActiveVideoExport]);

  const retryGeneration = async (course: CloudCourse) => {
    if (!course.generation_job_id || retryingId) return;
    setRetryingId(course.id);
    try {
      const response = await fetch(`/api/generation-jobs/${course.generation_job_id}`, {
        method: 'POST',
      });
      if (!response.ok) throw new Error('生成任务重新提交失败');
      await fetchCourses();
    } catch (retryError) {
      window.alert(getErrorMessage(retryError, '生成任务重新提交失败。'));
    } finally {
      setRetryingId(null);
    }
  };
  ...
  const visibleCourses = useMemo(() => {
    ...
    return courses.filter((course) => {
      ...
      const matchesQuery =
        !normalizedQuery ||
        [course.title, course.topic, course.author_name]
          ...
          >
            {filters.map((item) => (
              <button
                ...
                className={`border-b-2 pb-2 text-sm transition-colors ${filter === item.value ? 'border-slate-900 font-semibold text-slate-900' : 'border-transparent text-slate-500 hover:text-slate-800'}`}
              >
                {item.label} <span className="text-xs">{counts[item.value]}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      {visibleCourses.length === 0 ? (
        <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50/60 px-6 py-12 text-center">
          <p className="text-sm font-medium text-slate-700">
            {courses.length === 0 ? '还没有保存到云端的课程' : '没有匹配的课程'}
          </p>
          <p className="mt-2 text-sm text-slate-500">
            {courses.length === 0
              ? '创建课程后保存到云端，即可在这里集中管理。'
              : '请调整搜索关键词或筛选条件后重试。'}
          </p>
          {courses.length === 0 && (
            <Link
              href="/studio"
              className="mt-4 inline-flex h-9 items-center rounded-lg bg-emerald-600 px-3 text-sm font-medium text-white transition-colors hover:bg-emerald-700"
            >
              AI 创建课程
            </Link>
          )}
          ...
          <div className="min-w-[1120px]">
            ...
            {visibleCourses.map((course) => {
              ...
              const generationActive =
                course.generation_status === 'queued' || course.generation_status === 'running';
              const generationFailed =
                course.generation_status === 'failed' || course.generation_status === 'conflict';
              ...
              return (
                <div key={course.id} className="border-b border-slate-100 last:border-0">
                  <div className="grid grid-cols-[minmax(260px,2.5fr)_minmax(140px,1fr)_minmax(150px,1.3fr)_minmax(160px,1.3fr)_minmax(320px,1.8fr)] items-center gap-4 px-5 py-4 hover:bg-slate-50/70">
                    ...
                    >
                      ...
                      <p className="mt-1 truncate text-xs text-slate-400">
                        {isAdmin && course.author_name ? `作者：${course.author_name} · ` : ''}
                        更新于 {formatDate(course.updated_at)}
                      </p>
                    </button>
                    <div>
                      <CourseStatus
                        state={course.save_state}
                    ...
                    <button
                      ...
                      className="text-left"
                    >
                      <VideoStatus job={job} capability={videoCapability} />
                    </button>
                    <div className="truncate text-xs text-slate-500" title={activity}>
                      {activity}
                    </div>
                    <div className="flex min-w-0 flex-nowrap items-center gap-2 lg:justify-end">
                      {job?.status === 'completed' && job.downloadUrl && (
                        <a
                          ...
                          className="inline-flex h-8 shrink-0 items-center gap-1.5 whitespace-nowrap rounded-lg border border-emerald-200 bg-emerald-50 px-2.5 text-xs font-medium text-emerald-700 transition-colors hover:border-emerald-300 hover:bg-emerald-100"
                        >
                      ...
                      <button
                        ...
                        aria-label="预览课程"
                        title="预览课程"
                        className="inline-flex size-8 shrink-0 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-500 transition-colors hover:border-emerald-200 hover:bg-emerald-50 hover:text-emerald-700 disabled:cursor-not-allowed disabled:opacity-40"
                      >
                        <Eye className="size-4" />
                      </button>
                      {canManage && generationFailed && course.generation_job_id ? (
                        <button
                          type="button"
                          disabled={retryingId === course.id}
                          onClick={() => void retryGeneration(course)}
                          className="inline-flex h-8 shrink-0 items-center gap-1.5 whitespace-nowrap rounded-lg border border-red-200 bg-red-50 px-3 text-xs font-medium text-red-700 disabled:opacity-50"
                        >
                          <Loader2
                            className={`size-3.5 ${retryingId === course.id ? 'animate-spin' : ''}`}
                          />
                          {retryingId === course.id ? '重试中…' : '重试生成'}
                        </button>
                      ) : canManage ? (
                        <button
                          type="button"
                          ...
                          onClick={() =>
                            window.open(
                              `/classroom/${course.id}?editor=1`,
                              '_blank',
                              'noopener,noreferrer',
                            )
                          }
                          className="inline-flex h-8 shrink-0 items-center gap-1.5 whitespace-nowrap rounded-lg border border-slate-200 bg-white px-3 text-xs font-medium text-slate-700 transition-colors hover:border-slate-300 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50"
                        >
                          <Pencil className="size-3.5 text-slate-400" />
                          {generationActive ? '生成中' : '继续编辑'}
                        </button>
                      ) : null}
                      <div
                        ref={actionMenuId === course.id ? actionMenuRef : undefined}
                      ...
                      >
                        <button
                          ...
                          aria-label="更多操作"
                          title="更多操作"
                          className="inline-flex size-8 items-center justify-center rounded-lg text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-600"
                        >
                          <MoreHorizontal className="size-4" />
                        </button>
                        {actionMenuId === course.id && (
                          <div className="absolute right-0 z-30 mt-2 w-32 rounded-lg border border-slate-200 bg-white p-1 shadow-lg">
                            <button
                              type="button"
                              onClick={() => void shareCourse(course.id)}
                              disabled={sharingId === course.id}
                              className="flex w-full items-center gap-2 rounded px-2 py-2 text-left text-xs hover:bg-slate-50 disabled:opacity-50"
                            >
                  ...
                  {expanded && (
                    <div className="grid gap-3 border-t bg-slate-50 px-5 py-4 md:grid-cols-3">
                      <div className="rounded-lg border bg-white p-3">
                        <p className="text-xs font-medium text-slate-400">课程主题</p>
                        <p className="mt-2 text-sm font-medium text-slate-800">
                          {course.topic || '未填写课程主题'}
                        </p>
                      </div>
                      <div className="rounded-lg border bg-white p-3">
                        <p className="text-xs font-medium text-slate-400">云端保存</p>
                        <p className="mt-2 text-sm font-medium text-slate-800">
                          {course.save_state === 'ready'
                            ? '已保存到云端'
                            : course.save_state === 'draft'
                              ? '草稿待保存'
                              : '保存需要重试'}
                        </p>
                        <p className="mt-1 text-xs text-slate-500">课程资产由云端资产库统一管理</p>
                      </div>
                      <div className="rounded-lg border bg-white p-3">
                        <p className="text-xs font-medium text-slate-400">视频任务</p>
                        <div className="mt-2">
                          <VideoStatus job={job} capability={videoCapability} />
                        </div>
