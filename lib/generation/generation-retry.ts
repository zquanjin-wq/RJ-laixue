...
function retryableByMessage(value: unknown): boolean {
  ...
  return /rate limit|too many requests|timeout|timed out|failed to fetch|fetch failed|network|ECONNRESET|ECONNREFUSED|ECONNABORTED|ETIMEDOUT|ENOTFOUND|EPIPE|socket hang up/i.test(
    message,
  );
}
