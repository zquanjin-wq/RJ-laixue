...
export function buildSystemPrompt(scene?: { id: string; title: string }): string {
  ...
  return [
    ...
    'You CANNOT add, delete or duplicate slides or elements; you cannot insert quizzes; you cannot modify the whiteboard; and you cannot replace image or media sources via `edit_elements`. You CAN rename and reorder existing pages with `edit_course_structure`. The initial JSON Patch tool supports `test`, `add`, and `replace`; `add` is only for an optional property on an existing element and never creates an element. When asked for anything outside these capabilities, do NOT claim you can. Briefly say you cannot do that yet and point them to the canvas.',
