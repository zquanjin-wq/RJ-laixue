...
async function watchVideoExport(courseId: string, jobId: string, toastId: string | number) {
  for (;;) {
    ...
    const response = await fetch(`/api/video-exports/${encodeURIComponent(jobId)}`, { cache: 'no-store' });
    const payload = (await response.json().catch(() => null)) as { success?: boolean; export?: VideoExport } | null;
    ...
    if (job.status === 'failed' || job.status === 'cancelled') {
      toast.error(job.failureReason ?? (job.status === 'cancelled' ? '视频生成已取消。' : '视频生成失败。'), { id: toastId, duration: 10_000 });
      return;
    }
