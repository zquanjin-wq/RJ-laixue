 ...
 *   - externalizeImages: base64 → 腾讯云 COS
...
/**
 * Upload base64-inline images to COS.
  ...
  const externalizedPdfImages = await Promise.all(
    pdfImages.map(async (img) => {
      ...
      try {
        ...
        await storage.putObject(storagePath, buffer, mimeType);
        await courses.createAsset({
          ownerUserId: opts.callerUserId,
          kind: 'image',
        ...
        return {
          ...
          src: '',
          storagePath,
          publicUrl: `/api/course-assets/object?key=${encodeURIComponent(storagePath)}`,
        };
      } catch (e) {
