...
export async function listLearners() {
  const result = await getDatabasePool().query<{
    ...
    `SELECT u.id, p.display_name AS name, u.email, COALESCE(u.banned, false) AS disabled_at, p.created_at
...
export async function getLearner(userId: string) {
  const result = await getDatabasePool().query<{ id: string }>(
    `SELECT u.id
     FROM app.user_profiles p
     JOIN public."user" u ON u.id = p.user_id
     WHERE p.user_id = $1 AND p.role = 'learner'`,
    [userId],
  );
  return result.rows[0] ?? null;
}

export async function setLearnerDisabled(userId: string, disabled: boolean) {
  const result = await getDatabasePool().query(
    `UPDATE public."user" u
     SET banned = $2, "updatedAt" = now()
     FROM app.user_profiles p
     WHERE u.id = p.user_id AND u.id = $1 AND p.role = 'learner'`,
    [userId, disabled],
  );
  return result.rowCount === 1;
}

export async function createLearner(input: { email: string; name: string; password: string }) {
  const auth = getAuth();
