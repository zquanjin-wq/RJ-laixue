...
export async function requireAuthOrTeacher(
  ...
  try {
    ...
    log.error('requireAuthOrTeacher: failed to load current actor:', e);
    return {
      ok: false,
      response: apiError('SERVER_MISCONFIG', 500, 'Authentication service unavailable'),
    };
  }
  ...
  return { ok: true, user: { id: actor.userId, email: actor.email }, role };
}
