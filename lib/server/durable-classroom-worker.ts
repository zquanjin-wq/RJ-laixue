  ...
  try {
        ...
        onProgress: async (next) => {
          ...
          await jobs.appendEvent(lease, {
            ...
            details: {
              ...
              scenesGenerated: next.scenesGenerated,
    ...
    if (!finished && !(error instanceof GenerationLeaseLost)) {
      try {
        ...
        await jobs.fail(lease, failure.code, failure.message, isRetryableGenerationError(error));
      } catch (failure) {
...
async function runPptxAiClassroomJob(input: {
  ...
  const source = await new PptxSourceRepository(input.pool).getOwned(
    request.sourceId,
    input.lease.ownerUserId,
  );
  ...
  if (
    (content.stage as Stage & { pptxSource?: { sourceId?: string } }).pptxSource?.sourceId !==
    request.sourceId
  )
  ...
  const inspections = await checkpoints.run(
    'inspect_import',
    { courseId: course.id, scenes: content.scenes },
    async () => inspectImportedPptxPages(content.scenes!),
  );
  const scenes = request.applySafeRepairs
    ? applyPptxSafeTitleRepairs(content.scenes, inspections)
    : content.scenes;
  const model = await resolveModel({
    modelString: config.model.modelString,
      ...
      await callLLM(
        {
          ...
          maxOutputTokens: model.modelInfo?.outputWindow,
          maxRetries: 0,
        },
        'generate-classroom',
        undefined,
  ...
  await input.onProgress({
    ...
    progress: 20,
    message: '正在配置 AI 教师与伴学',
    scenesGenerated: 0,
    totalScenes: scenes.length,
  });
  const roster = await checkpoints.run(
    'roster',
    { title: content.stage.name, language: request.languageDirective, inspections },
    () =>
      generatePptxRoster({
  ...
  for (const [index, page] of inspections.entries()) {
    await input.onProgress({
      ...
      progress: 25 + Math.round((index / Math.max(inspections.length, 1)) * 65),
      message: `正在生成第 ${page.page} 页讲稿`,
  ...
  if (request.enableTTS) {
    ...
    const configuredInterval = Number.parseInt(process.env.PPTX_TTS_MIN_INTERVAL_MS ?? '6500', 10);
    ...
    for (const [index, target] of targets.entries()) {
      await input.onProgress({
        step: 'generating_tts',
        progress: 90 + Math.round((index / Math.max(targets.length, 1)) * 4),
        message: `正在生成第 ${index + 1}/${targets.length} 段配音`,
        scenesGenerated: scripts.length,
        totalScenes: scenes.length,
      });
      const generated = await checkpoints.run(
        `tts/${target.sceneId}/${target.actionId}`,
        target,
        async () => {
          const waitMs = Math.max(0, lastTtsCompletedAt + minIntervalMs - Date.now());
          if (waitMs > 0) await new Promise<void>((resolve) => setTimeout(resolve, waitMs));
          const result = await synthesizePptxSpeechWithRetry(
            {
            ...
            },
            {
              onRetry: async (attempt, delayMs) => {
                await input.onProgress({
                  step: 'generating_tts',
                  progress: 90 + Math.round((index / Math.max(targets.length, 1)) * 4),
                  message: `配音服务繁忙，${Math.ceil(delayMs / 1000)} 秒后重试（第 ${attempt} 次）`,
                  scenesGenerated: scripts.length,
                  totalScenes: scenes.length,
                });
              },
  ...
  const draft = {
    stage: applyPptxRoster(
      { ...content.stage, languageDirective: request.languageDirective },
      roster,
    ),
    scenes: voicedScenes,
  ...
  if (blocking.length) {
    throw new Error(
      `PPTX AI classroom quality validation failed: ${blocking.map((issue) => issue.code).join(', ')}`,
    );
  }
  const warnings = [
